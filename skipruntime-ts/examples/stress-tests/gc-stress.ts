...
async function runMemoryTest(
  ...
  for (let i = 0; i < cycles; i++) {
    ...
    if ((i + 1) % measureInterval === 0) {
      ...
      if (opts.trace) {
        console.log(
          `  [${testName}] cycle ${i + 1}: skipMem=${fmtBytes(mem)}`,
        );
      }
    }
  }
...
async function main(): Promise<void> {
  ...
  console.log("==================================================================");
  console.log("");
  console.log(`Running ${numTests} test${numTests > 1 ? "s" : ""}, estimated duration: ${estimatedDuration}.`);
  if (!opts.trace) {
    console.log("Tests run silently. Use --trace to see intermediate measurements.");
  }
  console.log("");
